...
class ItemDef {
  ...
  const ItemDef(this.id, this.label, {this.defaultOn = false});
}
...
class CategoryDef {
  ...
  bool get isSolo => items.isEmpty;
}

/// The master list, in display order. This order is used for both Simple
/// and Precise modes.
const List<CategoryDef> kBuiltInCategories = [
  CategoryDef('today', 'Today'),
  CategoryDef('mood', 'Mood'),
  CategoryDef('health', 'Health'),
  CategoryDef('mental', 'Mental'),
  CategoryDef('weather', 'Weather'),
  CategoryDef('memory', 'Memory'),
  CategoryDef('luck', 'Luck'),
  CategoryDef('flow_current', 'Flow and Current'),
...
/// For a solo category, treat it as having exactly one pseudo-item whose
/// id equals the category id — this lets storage/rating code use the
/// same (categoryId, itemId) shape everywhere, solo or not.
List<ItemDef> effectiveItems(CategoryDef cat) {
  if (cat.isSolo) return [ItemDef(cat.id, cat.label, defaultOn: true)];
  return cat.items;
}
