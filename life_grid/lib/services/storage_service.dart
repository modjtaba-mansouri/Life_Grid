...
class StorageService {
  ...
  /// Fully deletes a day's entry (ratings, counts, and note — everything).
  /// Always removed locally immediately; the Firestore copy is deleted
  /// best-effort right away (not queued for later if offline, since
  /// there's no real harm in a stray cloud doc lingering briefly — it's
  /// simply overwritten or re-deleted next time this runs online).
  Future<void> deleteEntry(String dateKey) async {
    ...
    try {
      await _db.deleteDocument('users/$_userSlug/entries/$dateKey');
