...
class ParametersService {
  ...
  /// Adult-only categories (currently just Intimacy) are excluded
  /// entirely — not shown, not toggleable — unless the account's
  /// stored birthday clearly indicates the user is 18+.
  Future<List<CategoryDef>> allCategories() async {
    final custom = await loadCustomCategories();
    var all = [...kBuiltInCategories, ...custom];
    if (!isAdult(StorageService.currentUser)) {
      all = all.where((c) => c.id != 'intimacy').toList();
    }
  ...
  Future<Set<String>> loadEnabledItems(String categoryId) async {
    ...
    final defaults = cat.first.items.where((i) => i.defaultOn).map((i) => i.id).toSet();
    // Period/PMS start enabled by default specifically for users whose
    // account gender is Female — available to everyone else too, just
    // not pre-checked, since gender alone shouldn't decide what's
    // relevant to someone's actual life.
    if (categoryId == 'health' &&
        (StorageService.currentUser?.gender ?? '').toLowerCase() == 'female') {
      defaults.addAll(['period', 'pms']);
    }
    return defaults;
  }
