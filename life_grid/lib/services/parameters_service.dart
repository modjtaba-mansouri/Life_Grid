...
class ParametersService {
  ...
  Future<List<CategoryDef>> loadCustomCategories() async {
    ...
    return list
        .map((e) => CategoryDef(e['id'], e['label'], isCustom: true))
        .toList();
  }

  Future<void> addCustomCategory(String label) async {
    final id = 'custom_${DateTime.now().millisecondsSinceEpoch}';
    final existing = await loadCustomCategories();
    existing.add(CategoryDef(id, label, isCustom: true));
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
      _customCategoriesKey,
      jsonEncode(existing.map((c) => {'id': c.id, 'label': c.label}).toList()),
    );
