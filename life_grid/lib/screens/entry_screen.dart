...
import '../widgets/counter_selector.dart';
...
class _EntryScreenState extends State<EntryScreen> {
  ...
  Future<void> _setItemCount(String categoryId, String itemId, int value) async {
    setState(() => _entry = _entry.withCount(categoryId, itemId, value));
  }
  ...
  List<Widget> _buildCategoryRows(CategoryDef cat) {
    ...
    if (visibleItems.isEmpty) {
      // No items enabled (or none added yet) — fall back to one combined
      // row so the category doesn't just disappear.
      return [
        StageSelector(
          label: cat.label,
          value: _entry.itemValue(cat.id, cat.id),
    ...
    return [
      ...
      for (final item in visibleItems)
        if (item.isCounter)
          CounterSelector(
            label: item.label,
            value: _entry.itemCount(cat.id, item.id) ?? 0,
            onChanged: (value) => _setItemCount(cat.id, item.id, value),
          )
        else
          StageSelector(
            label: item.label,
