...
enum _AddType { standalone, category }
...
class _ParametersScreenState extends State<ParametersScreen> {
  ...
  Future<void> _addCategory() async {
    final choice = await _askAddType();
    if (choice == null) return;
    final label = await _promptText(
      choice == _AddType.standalone ? 'New standalone item name' : 'New category name',
    );
    if (label == null || label.trim().isEmpty) return;
    await _params.addCustomCategory(label.trim(), standalone: choice == _AddType.standalone);
    _load();
  }

  Future<_AddType?> _askAddType() {
    return showDialog<_AddType>(
      context: context,
      builder: (context) => AlertDialog(
        backgroundColor: AppColors.bgElevated,
        title: Text('What do you want to add?', style: TextStyle(color: AppColors.text, fontSize: 15)),
        content: Text(
          'A standalone item is a single flat rating, like "Today" or "Mood". '
          'A category can hold its own sub-items (like "Family" holds "You Towards Them", '
          '"They Towards You", etc.) — you can add those items to it afterward.',
          style: TextStyle(color: AppColors.textDim, fontSize: 12),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(context, _AddType.standalone),
            child: Text('Standalone Item', style: TextStyle(color: AppColors.accent)),
          ),
          TextButton(
            onPressed: () => Navigator.pop(context, _AddType.category),
            child: Text('Category', style: TextStyle(color: AppColors.accent)),
          ),
        ],
      ),
    );
  ...
  Widget _categoryTile(CategoryDef cat, {required Key key}) {
    final hasItems = !cat.isStandalone;
